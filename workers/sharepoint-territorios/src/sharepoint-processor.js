...
export function parseSpreadsheet(buffer) {
  ...
  const getTerritory = (name) => {
    ...
    if (!territoryMap.has(canonicalName)) {
      territoryMap.set(canonicalName, {
        ...
        semiaridoContador: 0,
        futureSignals: { agriculturaFamiliar: null, gruposSubrepresentados: null },
  ...
  const processSheet = (sheetName) => {
    ...
    let iTipo = findColumnIndex(normalizedHeaders, ['tipo', 'natureza', 'categoria']);
    ...
    for (let r = headerRowIndex + 1; r < rows.length; r++) {
      ...
      let rowEntidades = 0;
      if (iEntidades !== -1) rowEntidades = toNumber(row[iEntidades]);
      if (rowEntidades <= 0) rowEntidades = 1;
      ...
      if (iIfdmTi !== -1) {
        ...
        if (ifdmTiRaw > 0) territory.desenvolvimento.ifdmTi = ifdmTiRaw;
      }
      ...
      if (cadeias.length > 0) {
        territory.cadeiasRows.push({ municipio, cadeias, municipioSatelite: satelite });
      }
      ...
      if (iAgriculturaFamiliar !== -1) {
        ...
        if (agricultura > 0) territory.futureSignals.agriculturaFamiliar = agricultura;
      }
  ...
  const territories = Array.from(territoryMap.values())
    .map((entry) => {
      ...
      return {
        ...
        semiaridoPercentual:
          entry.semiaridoContador > 0 ? entry.semiaridoAcumulado / entry.semiaridoContador : null,
  ...
  return { generatedAt: new Date().toISOString(), territories, summary };
}
